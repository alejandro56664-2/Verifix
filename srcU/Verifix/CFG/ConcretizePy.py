...
def add_indentation(indentLevel: int) -> str:
...
def concretize_op_unary(op:model.Op, var:str, indentLevel:int, ignoreSpl) -> Concrete:
    '''TODO: Update ListHead and ListTail - approximated for now'''
    expr1 = op.args[0]
    
    if op.name in ['!', '-', '+']:
        expr1_str = concretize_expr_rec(expr1, var, indentLevel, ignoreSpl)
        exprStr1 = expr1_str.exprStr
        if not (type(expr1) is model.Var or type(expr1) is model.Const):
            exprStr1 = '(' + expr1_str.exprStr + ')'
        return Concrete(add_indentation(indentLevel) + '{} {}'.format(op.name.replace('!', 'not'), exprStr1))
...
def concretize_op_binary(op:model.Op, var:str, indentLevel:int, ignoreSpl) -> Concrete:
    '''TODO: Update ListHead and ListTail - approximated for now'''
    expr1, expr2 = op.args[0], op.args[1]
    if op.name == '=':
        expr1_str, expr2_str = concretize_expr_rec(expr1, var, indentLevel, ignoreSpl), concretize_expr_rec(expr2, var, ignoreSpl)
        exprStr1, exprStr2 = expr1_str.exprStr, expr2_str.exprStr
        return Concrete('{}={}'.format(exprStr1, exprStr2))

    elif op.name in ['+', '-', '*', '/', '%', '<', '<=', '>', '>=', '==', '!=', '&&', '||']:
        expr1_str, expr2_str = concretize_expr_rec(expr1, var, indentLevel, ignoreSpl), concretize_expr_rec(expr2, var, indentLevel, ignoreSpl)
        exprStr1, exprStr2 = expr1_str.exprStr, expr2_str.exprStr
        if not (type(expr1) is model.Var or type(expr1) is model.Const):
            exprStr1 = '(' + expr1_str.exprStr + ')'
    ...
    #    return concretize_funcCall(op.name, op.args, var, ignoreSpl)
        ...
        return Concrete(add_indentation(indentLevel) + '({}){}'.format(expr1_str, expr2_str))
...
def concretize_op_nary(op:model.Op, var:str, indentLevel:int, ignoreSpl) -> Concrete:
    if op.name == 'StrFormat':
        ...
        return Concrete(add_indentation(indentLevel) + expr_str, flag_noAssign=True, flag_addEOL=False)
...
def concretize_op(op:model.Op, var:str, indentLevel:int, ignoreSpl) -> Concrete:
    if op.name in ['StrFormat', 'ite']:
        return concretize_op_nary(op, var, indentLevel, ignoreSpl)

    elif len(op.args) == 0:
        return concretize_op_noArgs(op, var, indentLevel, ignoreSpl)

    elif len(op.args) == 1:
        return concretize_op_unary(op, var, indentLevel, ignoreSpl)

    elif len(op.args) == 2:
        return concretize_op_binary(op, var, indentLevel, ignoreSpl)
...
def concretize_ret(expr:model.Expr, var:str, indentLevel:int) -> Concrete:
    ...
    expr_str = concretize_expr_rec(expr, var, 0, ignoreSpl=True)
...
def concretize_out(expr:model.Expr, var:str, indentLevel:int) -> Concrete:
    expr_str = concretize_expr_rec(expr, var, indentLevel, ignoreSpl=True)
    ...
    # Else, return the expr 
    return Concrete('print({})'.format(expr_str), flag_noAssign=True)


def concretize_expr_rec(expr:model.Expr, var:str, indentLevel:int, ignoreSpl) -> Concrete:
    ...
    # Special return case
        ...
        return concretize_ret(expr, var,  indentLevel)
    ...
    # Func call
        ...
        return concretize_funcCall_op(expr, var, indentLevel, ignoreSpl)

    # Op
    elif type(expr) is model.Op:
        return concretize_op(expr, var, indentLevel, ignoreSpl)

    raise raise_exceptionExpr(expr)

def concretize_expr_str(expr:model.Expr, var:str, indentLevel: int, ignoreSpl, flag_addEOL=True, flag_noAssign=False) -> str:
    expr_str = concretize_expr_rec(expr, var, indentLevel, ignoreSpl=ignoreSpl)
...
def concretize_cond(fnc:model.Function, loc:int, ignoreSpl):
    ...
    for var, expr in fnc.locexprs[loc]:
        ...
        # Add other statements by appending a comma "," at the end
            ...
            exprStr = concretize_expr_str(expr, var, 0, ignoreSpl, flag_addEOL=False, flag_noAssign=False)
            condExprStr += exprStr + ', '

    assert condVar is not None, 'Concretize: $cond variable does not exist' % (fnc.locexprs[loc])    
    exprStr = concretize_expr_str(condExpr, condVar, 0, ignoreSpl, flag_addEOL=False, flag_noAssign=True)
    condExprStr += exprStr

    return condExprStr


def concretize_block(fnc:model.Function, loc:int, seen:list, lines:list,  indentLevel:int, flag_addLines=True, flag_addEOL=True):
    ...
    if loc in fnc.locexprs:
        ...
        for (var, expr) in fnc.locexprs[loc]:
            if var == '$in':
                continue
            expr_str = concretize_expr_str(expr, var, indentLevel, ignoreSpl=False, flag_addEOL=flag_addEOL)
            if expr_str != '':
                blocks_str.append(add_indentation(indentLevel) + (expr_str))
...
def concretize_funcDef(fnc, lines):
    ...
    # TODO: ignore type
    params_str = ','.join([name for name, _ in fnc.params])
    # TODO: ignore fnc.rettype
    func_def = 'def {}({}): '.format(fnc.name, params_str)
...
def concretize_funcCall(name:str, args, var:str, indentLevel:int, ignoreSpl):
    exprStr = '{}({})'.format(name, 
        ','.join([concretize_expr_rec(arg, var, 0, ignoreSpl).exprStr for arg in args]))

    return Concrete(exprStr)

def concretize_funcCall_op(op:model.Op, var:str, indentLevel:int, ignoreSpl):
    return concretize_funcCall(op.args[0], op.args[1:], var, indentLevel, ignoreSpl)
...
def concretize_forLoop(fnc:model.Function, loc:int, locStops:List[int], seen:list, lines:list, indentLevel: int):
    locCond, locUpdate, locIn, locOut = get_forLoc(fnc, loc)
    condStr = concretize_cond(fnc, locCond, ignoreSpl=False)
    updateStr, flagBreak = concretize_block(fnc, locUpdate, seen, lines,  indentLevel + 1, flag_addLines=False, flag_addEOL=False)
    ...
    concretize_loc(fnc, locOut, locStops, seen, lines, indentLevel)
...
def concretize_whileLoop(fnc:model.Function, loc:int, locStops:List[int], seen:list, lines:list, indentLevel:int):
    
    locCond, locIn, locOut = get_whileLoc(fnc, loc)
    condStr = concretize_cond(fnc, locCond, ignoreSpl=False)
    # Add loop cond and update
    lines.append(add_indentation(indentLevel) + 'while {}:'.format(condStr))
    seen.extend([locCond])

    # Recurse inside loop body
    concretize_loc(fnc, locIn, locStops + [locOut], seen, lines, indentLevel+1)
    #lines.append('}')
...
def concretize_doWhileLoop(fnc:model.Function, loc:int, locStops:List[int], seen:list, lines:list, indentLevel: int):
...
def concretize_if(fnc:model.Function, loc:int, locStops:List[int], seen:list, lines:list, indentLevel:int):
    locCond, locIn, locOut, locElse = get_ifLoc(fnc, loc)
    condStr = concretize_cond(fnc, locCond, ignoreSpl=False)
    
    # Add if cond 
    lines.append(add_indentation(indentLevel) + 'if {}:'.format(condStr))
    seen.extend([locCond])
    ...
    concretize_loc(fnc, locIn, locStops + [locOut], seen, lines, indentLevel+1)
    # guard to add pass.
    add_pass_reserved_word(len_lines_before, lines, indentLevel + 1)

    # Add else-branch, if it exists
    if locElse is not None:
        lines.append(add_indentation(indentLevel) + 'else:')
        len_lines_before = len(lines)
        concretize_loc(fnc, locElse, locStops + [locOut], seen, lines, indentLevel+1)
        # guard to add pass. if
        add_pass_reserved_word(len_lines_before, lines, indentLevel + 1)

    # Recurse after if-cond 
    concretize_loc(fnc, locOut, locStops, seen, lines, indentLevel)

def add_pass_reserved_word(len_lines_before, lines, indentLevel):
    if len(lines) - len_lines_before == 0:
        lines.append(add_indentation(indentLevel) + 'pass')
...
def concretize_loc(fnc:model.Function, loc:int, locStops:List[int], seen, lines, indentLevel: int):
    ...
    if is_forLoop(fnc, loc):
        concretize_forLoop(fnc, loc, locStops, seen, lines, indentLevel)

    elif is_whileLoop(fnc, loc):
        concretize_whileLoop(fnc, loc, locStops, seen, lines, indentLevel)
        
    elif is_doWhileLoop(fnc, loc):
        concretize_doWhileLoop(fnc, loc, locStops, seen, lines, indentLevel)
    
    # Case-2: if-conditional 
    elif is_ifCond(fnc, loc):
        concretize_if(fnc, loc, locStops, seen, lines, indentLevel)
    ...
    # Case-3: Single non-branching block
        ...
        block_str, flagBreak = concretize_block(fnc, loc, seen, lines, indentLevel) # Concretize the block
        if not flagBreak: # Unless a return or break or continue statement was encountered
            if locTrue not in locStops: # Or some visitor wants to stop at a particular loc
                concretize_loc(fnc, locTrue, locStops, seen, lines, indentLevel) # Concretizing the next loc
...
def concretize_prog(prog:model.Program):
    ...
    for fnc in prog.fncs.values():
        ...
        concretize_loc(fnc, fnc.initloc, [fnc.endloc], seen, lines, indentLevel + 1)
